...
namespace org.abwe.RockMissions.DataSelect
{
    /// <summary>
    /// 
    /// </summary>
    [Description( "Selects Last Interaction with a Person" )]
    [Export( typeof( DataSelectComponent ) )]
    [ExportMetadata( "ComponentName", "Select the first or last Interaction date with a person" )]
    public class LastInteractionSelect : DataSelectComponent
    {
        #region Properties
        ...
        public override Expression GetExpression( RockContext context, MemberExpression entityIdProperty, string selection )
        {
            ...
            if ( selectionValues.Length >= 4 )
            {
                ...
                IQueryable<string> qry;
                if ( selectionMode == FirstLastInteraction.First )
                {
                    qry = new PersonService(context).Queryable().Select(p => interactionQry.Where(l => p.Aliases.Any(a => a.Id == l.EntityId))
                    .OrderBy(l => l.InteractionDateTime)
                    .Select(l => l.InteractionDateTime.ToString() + " (" + l.PersonAlias.Person.NickName + " " + l.PersonAlias.Person.LastName + ")")
                    .FirstOrDefault());
                }
                else
                {
                    qry = new PersonService( context ).Queryable().Select( p => interactionQry.Where( l => p.Aliases.Any(a => a.Id == l.EntityId))
                    .OrderByDescending(l => l.InteractionDateTime)
                    .Select(l => l.InteractionDateTime.ToString() + " (" + l.PersonAlias.Person.NickName + " " + l.PersonAlias.Person.LastName + ")")
                    .FirstOrDefault());
                }
