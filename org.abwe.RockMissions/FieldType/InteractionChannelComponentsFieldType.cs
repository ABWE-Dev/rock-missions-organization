...
namespace org.abwe.RockMissions.Field.Types
{
    ...
    /// </summary>
    public class InteractionChannelComponentsFieldType : Rock.Field.FieldType
    {
        private const string INTERACTION_CHANNEL = "interactionChannel";


        internal Dictionary<string, string> GetListSource(Dictionary<string, ConfigurationValue> configurationValues)
        {
            if (configurationValues.ContainsKey(INTERACTION_CHANNEL) && configurationValues[INTERACTION_CHANNEL].Value.AsInteger() != 0)
            {
                int interactionChannelConfiguration = configurationValues[INTERACTION_CHANNEL].Value.AsInteger();
                return new InteractionComponentService(new RockContext()).Queryable().Where(ic => ic.InteractionChannelId == interactionChannelConfiguration).OrderBy(a => a.Name)
                    .ToList().Where(ic => ic.IsAuthorized(Authorization.VIEW, UserLoginService.GetCurrentUser().Person)).ToDictionary(c => c.Guid.ToString(), c => c.Name);
            }
        ...
        public override List<Control> ConfigurationControls()
        {
            ...
            editControl.Label = "Interaction Channel";
            editControl.AutoPostBack = true;
            editControl.SelectedIndexChanged += OnQualifierUpdated;
            ...
            if (interactionChannels.Any())
            {
                ...
                {
                    editControl.Items.Add(new ListItem(interactionChannel.Name, interactionChannel.Id.ToString().ToUpper()));
                }
            }
        ...
        public override Dictionary<string, ConfigurationValue> ConfigurationValues(List<Control> controls)
        {
            ...
            description = "Select the base interaction channel";
            configurationValues.Add(INTERACTION_CHANNEL, new ConfigurationValue("Interaction Channel", description, string.Empty));

            if (controls != null && controls.Count > 1)
            {
                var ddlInteractionChannels = controls[1] as RockDropDownList;
                configurationValues[INTERACTION_CHANNEL].Value = ddlInteractionChannels.Visible ? ddlInteractionChannels.SelectedValue : string.Empty;
            }
        ...
        public override void SetConfigurationValues(List<Control> controls, Dictionary<string, ConfigurationValue> configurationValues)
        {
            ...
            {
                var ddlInteractionChannels = controls[1] as RockDropDownList;
                ddlInteractionChannels.SelectedValue = configurationValues.ContainsKey(INTERACTION_CHANNEL) ? configurationValues[INTERACTION_CHANNEL].Value : string.Empty;
            }
        }
        ...
        public override List<string> ConfigurationKeys()
        {
            ...
            configKeys.Add(INTERACTION_CHANNEL);
            return configKeys;
        }
