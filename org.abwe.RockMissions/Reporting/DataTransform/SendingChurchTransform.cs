...
namespace org.abwe.RockMissions.Reporting
{
    /// <summary>
    /// Person Children Transformation
    /// </summary>
    [Description( "Transform result to Sending Churches" )]
    [Export( typeof( DataTransformComponent ) )]
    [ExportMetadata( "ComponentName", "Person Sending Church Transformation" )]
    public class SendingChurchTransform : DataTransformComponent<Rock.Model.Person>
    {
        /// <summary>
        /// Gets the title.
        /// </summary>
        ...
        private Expression BuildExpression( IService serviceInstance, IQueryable<int> idQuery, ParameterExpression parameterExpression )
        {
            ...
            int sentMissionaryRoleId = GroupTypeCache.Get(org.abwe.RockMissions.SystemGuid.GroupType.GROUPTYPE_CHURCH.AsGuid()).Roles.Where( a => a.Guid == org.abwe.RockMissions.SystemGuid.GroupTypeRole.GROUPROLE_SENT_MISSIONARY.AsGuid() ).Select( a => a.Id ).FirstOrDefault();
            int churchRoleId = GroupTypeCache.Get(org.abwe.RockMissions.SystemGuid.GroupType.GROUPTYPE_CHURCH.AsGuid()).Roles.Where( a => a.Guid == org.abwe.RockMissions.SystemGuid.GroupTypeRole.GROUPROLE_CHURCH.AsGuid()).Select( a => a.Id ).FirstOrDefault();

            var qry = new PersonService( (RockContext)serviceInstance.Context ).Queryable()
                .Where( p => p.Members.Where( a => a.GroupRoleId  == churchRoleId )
                    .Any( a => a.Group.Members
                    .Any( c => c.GroupRoleId == sentMissionaryRoleId && idQuery.Contains( c.PersonId ) ) ) );
